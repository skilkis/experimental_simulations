...
for i=1:length(fnBAL) % loop over the files that are to be loaded
    ...
    % give status update
    disp(['Processing balance data; configuration ''',BAL.config{i},'''; filename ''',fnBAL{i},'''.']);
    % load zero-measurement data (BAL0)
    BAL.windOff.(BAL.config{i}) = processing.bal.readzerodata(diskPath,fn0{i},idxB);
    % load measurement data (BAL)
    BAL.windOn.(BAL.config{i}) = processing.bal.readdata([diskPath,fnBAL{i}],idxB);
